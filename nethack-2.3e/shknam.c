...
static const char *shkbooks[] = {
    ...
    "Lugnaquillia",   "Enniscorthy", "Gweebarra",
    "Kittamagh",      "Nenagh",      "Sneem",
    "Ballingeary",    "Kilgarvan",   "Cahersiveen",
    "Glenbeigh",      "Kilmihil",    "Kiltamagh",
    "Droichead Atha", "Inniscrone",  "Clonegal",
    "Lisnaskea",      "Culdaff",     "Dunfanaghy",
    "Inishbofin",     "Kesh",        ""
};
...
static const char *shkrings[] = {
    ...
    "Svaving", "Tapper", "Terwen", "Wirix", "Ypey",
    /* Skandinaviske navne */
...
static const char *shkgeneral[] = {
    ...
    /* N. Canada */
    "Aklavik", "Inuvik", "Tuktoyaktuk", "Chicoutimi", "Ouiatchouane",
    "Chibougamau", "Matagami", "Kipawa", "Kinojevis", "Abitibi", "Maganasipi",
    /* Iceland */
 ...
 * array. See mkroom.h for the structure definition. Typically, you'll have to
 * lower some or all of the probability fields in old entries to free up some
...
struct shclass shtypes[] = {
...
#endif
      D_SHOP,
      { { 100, RANDOM_SYM }, { 0, 0 }, { 0, 0 } },
      shkgeneral },
    { "used armor dealership",
      ARMOR_SYM,
      14,
      D_SHOP,
      { { 90, ARMOR_SYM }, { 10, WEAPON_SYM }, { 0, 0 } },
      shkarmors },
    ...
    { "liquor emporium",
      ...
      10,
      D_SHOP,
      { { 100, POTION_SYM }, { 0, 0 }, { 0, 0 } },
      shkliquors },
    { "antique weapons outlet",
      WEAPON_SYM,
      5,
      D_SHOP,
      { { 90, WEAPON_SYM }, { 10, ARMOR_SYM }, { 0, 0 } },
      shkweapons },
    { "delicatessen",
      FOOD_SYM,
      5,
      D_SHOP,
      { { 95, FOOD_SYM }, { 5, POTION_SYM }, { 0, 0 } },
      shkfoods },
    { "jewelers",
      RING_SYM,
      3,
      D_SHOP,
      { { 90, RING_SYM }, { 10, GEM_SYM }, { 0, 0 } },
      shkrings },
...
saleable(int nshop, struct obj *obj)
{
    ...
    if (shtypes[nshop].symb == RANDOM_SYM)
        return (1);
    else {
        for (i = 0; shtypes[nshop].iprobs[i].iprob; i++)
            if (shtypes[nshop].iprobs[i].itype < 0) {
                if (shtypes[nshop].iprobs[i].itype == -obj->otyp)
                    return (1);
            } else if (shtypes[nshop].iprobs[i].itype == obj->olet)
                return (1);
    }
    return (0);
}
