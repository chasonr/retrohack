...
register struct obj *otmp;
{
    ...
    case SPE_FORCE_BOLT:
        ...
        if (u.uswallow || rnd(20) < 10 + mtmp->data->ac) {
            ...
            hit((otmp->otyp == WAN_STRIKING) ? "wand" : "spell", mtmp,
                exclam(tmp));
            resist(mtmp, otmp->olet, tmp, TELL);
        } else
            miss((otmp->otyp == WAN_STRIKING) ? "wand" : "spell", mtmp);
        break;
...
register struct obj *obj, *otmp; /* returns TRUE if sth was done */
{
    ...
    if (obj == uball || obj == uchain)
        ...
        case SPE_POLYMORPH:
...
#ifdef DGKMOD
            otmp2 = mkobj_at((obj->otyp == ROCK || obj->otyp == ENORMOUS_ROCK)
                                 ? GEM_SYM
                                 : obj->olet,
                             obj->ox, obj->oy);
            otmp2->quan = obj->quan;
            ...
            if ((otmp2->otyp == WAN_WISHING) && (obj->spe > 3))
                otmp2->spe = 3;
    ...
    if (objects[obj->otyp].bits & IMMEDIATE) {
        ...
        default:
            ...
            if ((int) obj->otyp >= SPE_MAGIC_MISSILE) {
                buzz((int) obj->otyp - SPE_MAGIC_MISSILE + 10, u.ux, u.uy,
                     u.dx, u.dy);
            } else
#endif

                buzz((int) obj->otyp - WAN_MAGIC_MISSILE, u.ux, u.uy, u.dx,
                     u.dy);
            break;
        }
...
register int dx, dy;
{
    ...
    if (u.uswallow) {
        ...
        if (!u.ustuck)
            ...
            pline("The %s rips into %s%s", fltxt, monnam(u.ustuck),
                  exclam(tmp));
        return;
    }
    if (type < 0)
                ...
                if (mon->mhp < 1) {
                    if (type < 0) {
                        if (cansee(mon->mx, mon->my))
                            pline("%s is killed by the %s!", Monnam(mon),
                                  fltxt);
                        mondied(mon);
                    } else
                        killed(mon);
...
register int type;
{
    ...
    case -16: /* poison breath */
        if (index("abcghikqsuvxyADFQSVWXZ&", mon->data->mlet))
            break;
        tmp = d(6, 6);
        break;
    case 7:   /* acid wand */
    case 17:  /* acid spell */
    case -17: /* acid breath */
        if (index("a", mon->data->mlet))
            break;
        tmp = d(6, 6);
        break;
    }
    if (type >= 0 && type != 20)
        if (resist(mon, (type < 10) ? '/' : '+', 0, NOTELL))
            tmp /= 2;
    mon->mhp -= tmp;
    return (tmp);
}
...
register struct obj *obj;
{
    ...
    if (obj->olet == FOOD_SYM && obj->otyp > CORPSE) {
        ...
        default:
            let = CORPSE_I_TO_C(obj->otyp);
        }
        delobj(obj);
        /* Originally there was a bug which caused the object not to be erased
           from the screen.  This happened because first the monster got
           created, then the corpse removed.  Although delobj() called
           unpobj(), the object didn't get erased from the screen because the
           monster was sitting on top of it.  Solution: place the delobj()
           call before the mkmon() call. */
...
burn_scrolls()
{
    ...
    if (cnt) {
        pline("%s of your scrolls catch%s fire!", cnt == 1 ? "One" : "Some",
              cnt == 1 ? "es" : "");
        if (Fire_resistance)
...
register int damage, tell;
{
    ...
    if (resisted) {
        if (tell)
            pline("The %s resists!", mtmp->data->mname);
        mtmp->mhp -= damage / 2;
    } else
#endif
        mtmp->mhp -= damage;
