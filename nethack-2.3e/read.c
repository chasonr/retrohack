...
doread()
{
    ...
    if (scroll->otyp == FORTUNE_COOKIE) {
        ...
        useup(scroll);
        return (1);
    } else if (scroll->olet != SCROLL_SYM) {
...
register struct obj *sobj;
{
    ...
    case SCR_ENCHANT_ARMOR: {
        ...
        pline("Your %s glows %s for a moment.", objects[otmp->otyp].oc_name,
              Hallucination ? hcolor() : "green");
        otmp->cursed = 0;
        otmp->spe++;
        break;
...
#endif
    {
        ...
        while (cnt--)
            ...
            if (wizard) {
                ...
                cmlet = buf[0];
                for (crmonst = mons;
                     crmonst->mlet != cmlet && crmonst != PM_EEL; crmonst++)
                    ;
    ...
    case SCR_ENCHANT_WEAPON:
        if (uwep && uwep->olet == WEAPON_SYM && confused) {
            ...
            pline("Your %s covered by a shimmering %s shield!",
                  aobjnam(uwep, "are"), Hallucination ? hcolor() : "gold");
            uwep->rustfree = 1;
        } else if (!chwepon(sobj, 1)) /* tests for !uwep */
            return (1);
        break;
    case SCR_DAMAGE_WEAPON:
        if (uwep && uwep->olet == WEAPON_SYM && confused) {
            /* olet check added 10/25/86 GAN */
            pline("Your %s %s for a moment.", aobjnam(uwep, "glow"),
                  Hallucination ? hcolor() : "purple");
            uwep->rustfree = 0;
        } else if (!chwepon(sobj, -1)) /* tests for !uwep */
            return (1);
    ...
    case SCR_GOLD_DETECTION:
        ...
        if (confused) {
            ...
            outtrapmap:
                ...
                for (ttmp = ftrap; ttmp; ttmp = ttmp->ntrap)
                    at(ttmp->tx, ttmp->ty,
                       Hallucination ? rndobjsym() : GOLD_SYM);
                prme();
...
#endif
    {
        ...
        if (!ct && !ctu) {
            ...
            for (obj = fobj; obj; obj = obj->nobj)
                if (obj->olet == foodsym)
                    at(obj->ox, obj->oy,
                       Hallucination ? rndobjsym() : FOOD_SYM);
            prme();
    ...
    case SCR_FIRE: {
        ...
        /*
         * Note: This case was modified 11/4/86 by DKC to eliminate the
         * problem with reading a scroll of fire while confused or resistant
         * to fire.  Formerly, the code failed to initialize the variable
         * "num" in these cases, resulting in monsters being hit for a
         * possibly large (and possibly negative) damage. The actions taken
         * now are: If the player is fire resistant, monsters take the normal
         * damage (1-6 except for Y's and F's), and the player is unaffected.
         */
        known = TRUE;
        if (confused) {
            if (Fire_resistance)
