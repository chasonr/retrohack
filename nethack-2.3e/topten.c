...
#define HUP
    ...
    fprintf(lfile, "%6s %d %d %d %d %d %ld %c%c %s,%s\n", t0->date, t0->uid,
            t0->level, t0->maxlvl, t0->hp, t0->maxhp, t0->points, t0->plchar,
            t0->sex, t0->name, t0->death);
    fclose(lfile);
    (void) unlink(loglock);
lgend:;
#endif
    ...
    for (rank = 1;;) {
        if (fscanf(rfile, "%6s %d %d %d %d %d %ld %c%c %[^,],%[^\n]",
                   t1->date, &t1->uid, &t1->level, &t1->maxlvl, &t1->hp,
                   &t1->maxhp, &t1->points, &t1->plchar, &t1->sex, t1->name,
                   t1->death)
                != 11
            || t1->points < POINTSMIN)
            t1->points = 0;
    ...
    for (rank = 1; t1->points != 0; rank++, t1 = t1->tt_next) {
        if (flg)
            fprintf(rfile, "%6s %d %d %d %d %d %ld %c%c %s,%s\n", t1->date,
                    ...
                    t1->points, t1->plchar, t1->sex, t1->name, t1->death);
        if (done_stopprint)
            continue;
...
int so;
{
    ...
    if (!strncmp("escaped", t1->death, 7)) {
        ...
        if (!strncmp(t1->death, "quit", 4)) {
            ...
            Sprintf(eos(linebuf), ", killed"), killed = TRUE;
