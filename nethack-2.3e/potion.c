...
register struct obj *otmp;
{
    ...
    case POT_HALLUCINATION:
        if (Hallucination)
            ...
            pline("Oh wow!  Everything looks so cosmic!");
    ...
    case SPE_DETECT_MONSTERS:
        ...
        if (!fmon) {
            ...
            for (mtmp = fmon; mtmp; mtmp = mtmp->nmon)
                if (mtmp->mx > 0)
                    at(mtmp->mx, mtmp->my,
                       Hallucination ? rndmonsym() : mtmp->data->mlet);
            prme();
    ...
    case SPE_DETECT_TREASURE:
        ...
        outobjmap:
            ...
            for (objs = fobj; objs; objs = objs->nobj)
                at(objs->ox, objs->oy,
                   Hallucination ? rndobjsym() : objs->olet);
    ...
    case POT_SICKNESS:
        ...
        if (Poison_resistance)
            pline("(But in fact it was biologically contaminated orange "
                  "juice.)");
#ifdef KAA
        if (pl_character[0] == 'H')
...
register struct obj *obj;
{
    ...
    if (!isyou && !rn2(3))
        ...
        case POT_BLINDNESS:
            mon->mblinded |=
                64 + rn2(32) + rn2(32) * !resist(mon, '!', 0, NOTELL);
            break;
#ifdef KAA
        case POT_HOLY_WATER:
