...
delay_output()
{
    ...
    else if (ospeed > 0 || ospeed < SIZE(tmspc10))
        if (CM) {
...
init_hilite()
{
    ...
    len = strlen(HI) - 1;
    if (len > 3)
        if (isdigit(HI[len - 1]) && isdigit(HI[len - 2])
            && HI[len - 2] != 3) {
            backg = HI[len - 1] - '0';
    ...
    if (mfore == backg || ofore == backg) {
        if (len < 7)
            ...
            if (HI[2] == '3')
                ...
                mfore = ofore = WHITE; /* give up! */
        }
    }

    HI_MON = (char *) alloc(sizeof("E[0;33;44;54m"));
    sprintf(HI_MON, "%c[%d;3%d;4%dm", ESC, HILITE_ATTRIB, mfore, backg);
    HI_OBJ = (char *) alloc(sizeof("E[0;33;44;54m"));
    sprintf(HI_OBJ, "%c[%d;3%d;4%dm", ESC, HILITE_ATTRIB, ofore, backg);
}
