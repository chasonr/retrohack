...
register struct monst *magr, *mdef;
{
    ...
    if (vis) {
        ...
        if (farq != far_noise || moves - noisetime > 10) {
            far_noise = farq;
            noisetime = moves;
            pline("You hear some noises%s.", farq ? " in the distance" : "");
        }
    }
#ifdef STOOGES
    if (hit && magr->isstooge) {
        if (!rn2(6) && !index("afgvyF", mdef->data->mlet)) {
            ...
            switch (rn2(100)) {
            case 0:
                pline("%s is shoved!", Monnam(mdef));
                break;
            case 1:
                pline("%s is kicked!", Monnam(mdef));
                break;
            case 2:
                pline("%s is slapped!", Monnam(mdef));
                break;
            case 3:
                pline("%s is slugged!", Monnam(mdef));
                break;
            case 4:
                pline("%s is punched!", Monnam(mdef));
                break;
            case 5:
                pline("%s is pinched!", Monnam(mdef));
                break;
            case 6:
                pline("But %s dodges!", monnam(mdef));
                break;
            case 7:
                pline("But %s ducks!", monnam(mdef));
                break;
            case 8:
                pline("%s gets a black eye!", Monnam(mdef));
                break;
            case 9:
                pline("%s gets a bloody nose!", Monnam(mdef));
                break;
            case 10:
                pline("%s gets a broken tooth!", Monnam(mdef));
                break;
            }
        if (!rn2(2))
            stoogejoke();
    ...
    if (hit) {
        if (magr->data->mlet == 'c' && !magr->cham) {
            ...
            if (vis)
                ...
                pline("You have a peculiarly sad feeling for a moment, then "
                      "it passes.");
            monstone(mdef);
            hit = 2;
...
#endif
    {
        ...
        if (!(pd->mlet == 'r' && dlevel < 4))
            ...
            if (!(pd->mlet == '&' && mdef->isdjinni)) /* no djinni corpse */
                if (!(pd->mlet == 'G'
                      && mdef->isgremlin)) /* no gremlin corpse */
                    if (letter(pd->mlet) && rn2(3)) {
                        if (pd->mlet == '1')
                            panic("mondied: making obj '1'");
#ifndef RPH
...
register int thrown;
{
    ...
    if (!obj) {
        ...
        } else if (index("P,'", u.usym) && u.uhunger < 1500 && !u.uswallow
                   && mon->data->mlet != 'c') {
        ...
        if (obj == uwep && (obj->otyp > SPEAR || obj->otyp < BOOMERANG))
            ...
            tmp += obj->spe;
#ifdef KAA
            if (obj->olet == WEAPON_SYM && obj->dknown
                && index("VWZ &", mon->data->mlet))
                tmp += rn2(4);
#endif
...
#ifdef BVH
        if (!strcmp(ONAME(obj), "Excalibur"))
            tmp += rnd(10);
        else
#endif
            if (obj->otyp == KATANA && !strcmp(ONAME(obj), "Snickersnee"))
            tmp += rnd(5);
    ...
    if (!hittxt) {
        if (thrown)
            ...
            hit(xname(obj) /* or: objects[obj->otyp].oc_name */, mon,
                exclam(tmp));
        else if (Blind)
            pline("You hit it.");
        else
...
register struct monst *mtmp;
{
    ...
    if (!Blind && !Confusion && !Hallucination && mdat->mlet == 'd'
        && mtmp->mtame) {
    ...
    if (mtmp->mhide && mtmp->mundetected) {
        ...
        if ((obj = o_at(mtmp->mx, mtmp->my)) && !Blind)
            pline("Wait! There's %s hiding under %s!", defmonnam(mtmp),
                  doname(obj));
        return (TRUE);
    }
#ifdef KAA
    ...
    if (uwep) {
        ...
        if (uwep->olet == WEAPON_SYM && uwep->dknown
            && index("VWZ &", mtmp->data->mlet))
            tmp += 2;
#endif
        ...
        if (uwep->otyp == TWO_HANDED_SWORD)
            tmp -= 1;
        else if (uwep->otyp == KATANA)
            tmp += 1;
        else if (uwep->otyp == DAGGER || uwep->otyp == SHURIKEN)
            tmp += 2;
        ...
        if (mdat->mlet == 'a') {
            if (rn2(2)) {
                if (Blind)
                    ...
                    pline("You are splashed by %s's acid!", monnam(mtmp));
                if (u.usym != 'a') {
                    losehp_m(rnd(6), mtmp);
                    if (!rn2(30))
                        corrode_armor();
