...
domove()
{
    ...
    if (u.dx && u.dy && IS_ROCK(levl[u.ux][u.uy + u.dy].typ)
        && IS_ROCK(levl[u.ux + u.dx][u.uy].typ) && invent
        && inv_weight() + 40 > 0) {
...
int all;
{
    ...
    if (!all) {
        ...
        ilets[iletct] = 0;
        while (otmp) {
            if (!index(ilets, otmp->olet) && otmp->ox == u.ux
                && otmp->oy == u.uy) {
        ...
        while ((sym = *ip++) != '\0') {
            ...
            if (sym == 'a')
                ...
                pline("There are no %c's here.", sym);
        }
    }
    ...
    for (x = u.ux - 1; x <= u.ux + 1; x++)
        for (y = u.uy - 1; y <= u.uy + 1; y++) {
                ...
                continue;
            {
                ...
                if (sym == VWALL_SYM || sym == HWALL_SYM || sym == ROOM_SYM
                    || sym == STONE_SYM || IS_CORNER(sym))
                    continue;
...
#ifdef KAA
    if (u.usym != '@')
        return (mons[u.umonnum].mlevel - 3);
#endif
    if (u.ustr == 3)
        return (-3);
    else if (u.ustr < 6)
...
inv_weight()
{
    ...
    if (Levitation) /* pugh@cornell */
        ...
        if (u.ustr < 19)
            carrcap += 5 * u.ustr;
        if (u.ustr > 18)
            carrcap += u.ustr - 18 + 90;
        if (u.ustr > 68)
            carrcap += u.ustr - 68;
        if (u.ustr > 93)
            carrcap += u.ustr - 93;
        if (u.ustr > 108)
            carrcap += 2 * (u.ustr - 108);
        if (u.ustr > 113)
            carrcap += 5 * (u.ustr - 113);
        if (u.ustr == 118)
            carrcap += 100;
#endif
