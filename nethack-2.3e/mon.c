...
movemon()
{
    ...
    if (warnlevel >= 0)
        if (warnlevel > lastwarnlev || moves > lastwarntime + 5) {
            ...
            pline("%s %s!", rr,
                  Hallucination ? hcolor() : warnings[warnlevel]);
            lastwarntime = moves;
                                ...
                                /* we cannot avoid traps of an unknown kind */
                                {
                                    ...
                                    if (ttmp) {
                                        ...
                                        tt = (ttmp->ttyp < TRAPNUM
                                              && ttmp->ttyp > 0);
                                        /* below if added by GAN 02/06/87 to
                                         * avoid traps out of range
                                         */
                                        if (!(tt & ALLOW_TRAPS)) {
                                            impossible(
                                                "A monster looked at a very "
                                                "strange trap of type %d.",
                                                ttmp->ttyp);
                                            continue;
                                        }
                                        if (mon->mtrapseen & tt) {
...
register char *string, *pname;
{
    ...
    if (Blind) {
        if (plural)
            pline("They were poisoned.");
        else
            pline("It was poisoned.");
    } else {
        if (plural)
            pline("The %s were poisoned!", string);
        else
...
#define MAXMONNO 100
    ...
    if (mdat->ac < 3)
        tmp += 2 * (7 - mdat->ac);
    if (index("AcsSDXaeRTVWU&In:P389", mdat->mlet))
        tmp += 2 * mdat->mlevel;
    ...
    if (old_nlth > 0)
        (void) strcpy(old_name, NAME(mtmp));
#endif
    ...
    if (tmp == 'm') { /* he killed a minotaur, give him a wand of digging */
        ...
        if (tmp == '&')
        (void) mkobj_at(0, x, y);
    else if (stoned == FALSE
             && (!letter(tmp) || (!index("9&1", tmp) && !rn2(3))))
        tmp = 0;
    if (dest & 2) {
        newsym(x, y);
        return;
     ...
     *  not make a corpse. */
#ifdef KOPS
    if (mdat->mlet == 'K' && !strcmp(mdat->mname, "kobold") && tmp)
        tmp2 = 0;
#endif
#ifdef ROCKMOLE
    if ((mdat->mlet == 'r' && dlevel < 4) && tmp)
        tmp2 = 0;
#endif
#endif
    if (!ACCESSIBLE(levl[x][y].typ)) {
        ...
        if (stoned) {
            ...
            if (cansee(x, y))
                atl(x, y,
                    Hallucination ? rndobjsym() : objects[typetmp].oc_olet);
        } else
#endif
            ...
            if (letter(tmp))
                obj2 = mk_named_obj_at(tmp, x, y, old_name, old_nlth);
#ifdef KOPS
            else if (mdat->mlet == 'K')
                obj2 = mksobj_at((rn2(2) ? CLUB : (rn2(2)) ? WHISTLE : BADGE),
                                 x, y);
#endif
            else
...
register struct permonst *mdat;
{
    ...
    if (mdat == 0) {
        ...
        mdat = &mons[CMNUM - 1
                     - rn2((CMNUM - 17) - (CMNUM - 20) * dlevel / MAXLEVEL)];
    }
#endif
    if (mdat == mtmp->data)
    ...
    if (mhp > hpd && mtmp->mhp < hpn)
        mtmp->mhp = 127;
    /* Not totally foolproof.  A 2HD monster with 80 HP that changes into a
       6HD monster that really should have 240 and actually should have 127,
       the maximum possible, will wind up having 113.  */
    if (!mtmp->mhp)
        mtmp->mhp = 1;
    /* Unlikely but not impossible; a 1HD creature with 1HP that changes into
       a 0HD creature will require this statement */
    mtmp->data = mdat;
    /* and the same for maximum hit points */
    hpn = mtmp->mhpmax;
    mtmp->mhpmax = (hpn * mhp) / hpd;
    if (mhp > hpd && mtmp->mhpmax < hpn)
        mtmp->mhp = 127;
    if (!mtmp->mhp)
        mtmp->mhp = 1;
...
register struct monst *mtmp;
{
    if (mtmp->data->mlet == 'M' && !mtmp->mimic && !mtmp->cham && !mtmp->mcan
        ...
        mtmp->mimic = 1;
        mtmp->mappearance =
            (levl[mtmp->mx][mtmp->my].typ == DOOR) ? DOOR_SYM : GOLD_SYM;
        return (1);
    }

    if (mtmp->data->mlet == 'p' && !mtmp->cham && !mtmp->mcan
        && !cansee(mtmp->mx, mtmp->my) && !rn2(3)) {
