...
gotone:
    ...
    if (index("w:", mons[mntmp].mlet) || !rn2(5)) {
        ...
        /* If it was u.uhpmax*u.ulevel/tmp+9-rn2(19), then a 1st level
           character with 16 hp who polymorphed into a 3rd level one would
           have an average of 48 hp.  */
    ...
    newname:
        ...
        if (index("CDelmoPTUVXYz9", u.usym))
            u.ustr = u.ustrmax = 118;
        if (u.usym == 'D')
            u.mhmax = 80;
        else if (!(mons[mntmp].mlevel))
            u.mhmax = rnd(4);
        else
            u.mhmax = d(mons[mntmp].mlevel, 8);
        u.mh = u.mhmax;
        ...
        if (u.usym == 'N')
            pline("Use the command #remove if you have to remove an iron "
                  "ball.");
        find_ac();
    }
...
dobreathe(void)
{
    if (u.usym == 'D') {
        ...
        if (rn2(4))
            ...
            buzz(20, u.ux, u.uy, u.dx, u.dy);
        /* Changes must be made in zap.c to accommodate this. */
    } else
        pline("You do not have the ability to breathe fire!");
    return (1);
}
