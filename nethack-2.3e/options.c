...
boolean from_env;
{
    ...
    if (!strncmp(opts, "fixinv", 4)) {
        ...
        if (!from_env && flags.invlet_constant)
            reassign();
        return;
    }
...
#ifdef SAFE_ATTACK
        if (flags.confirm)
            (void) strcat(buf, "confirm,");
#endif
#ifdef DGKMOD
        if (flags.pickup)
            (void) strcat(buf, "pickup,");
        if (flags.silent)
            (void) strcat(buf, "silent,");
#endif
#ifdef DGK
        if (flags.rawio)
            (void) strcat(buf, "rawio,");
        if (flags.IBMBIOS)
            (void) strcat(buf, "IBMBIOS,");
        if (flags.DECRainbow)
            (void) strcat(buf, "DECRainbow,");
#endif
        if (flags.time)
...
option_help()
{
    ...
    (void) sprintf(buf, "        Net%s Options Help:",
                   ...
                   "Quest");
#endif
    if (page_line("") || page_line(buf) || page_line(""))
        goto quit;

#ifdef DGK
    (void) sprintf(buf, "To set options use OPTIONS=<options> in %s",
                   configfile);
    Page_line(buf);
#else
    Page_line(
        "To set options use `HACKOPTIONS=\"<options>\"' in your environment");
#endif

    Page_line("or give the command \"O\" followed by the line <options> "
              "while playing.");
    Page_line("Here <options> is a list of options separated by commas.");
    Page_line("");

#ifdef DGK
    Page_line("Boolean options are confirm, pickup, rawio, silent, sortpack, "
              "time, IBMBIOS,");
    Page_line("and DECRainbow.  These can be negated by prefixing them with "
              "'!' or \"no\".");
#else
    Page_line("Boolean options are rest_on_space, news, time, null "
              "tombstone, and (fe)male,");
    ...
    Page_line("The `endgame' option is followed by a description of which "
              "parts of");
