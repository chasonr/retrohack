...
char *argv[];
{
    ...
    if ((fd = open(SAVEF, 0)) >= 0
        ...
        if (!dorecover(fd))
            goto not_recovered;
        pline("Hello %s%s, welcome to %s!", (Badged) ? "Officer " : "",
              plname, gamename);
        flags.move = 0;
    } else {
    not_recovered:
        ...
        if (wizard && dlevel == 1)
#ifdef STOOGES
            pline("The wiz is at %d, the medusa is at %d, and the stooges "
                  "are at %d",
                  u.wiz_level, u.medusa_level, u.stooge_level);
#else
            pline("The wiz is at %d, and the medusa at %d", u.wiz_level,
                  u.medusa_level);
#endif
#endif
        pickup(1);
