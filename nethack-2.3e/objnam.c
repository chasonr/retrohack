...
register struct obj *obj;
{
...
#ifdef KAA
    if (!obj->dknown && !Blind && obj->olet != WEAPON_SYM)
        obj->dknown = 1;
#else
    if (!obj->dknown && !Blind)
    ...
    case GEM_SYM:
        ...
        if (!nn) {
            ...
            if (un) {
                if (!pl)
                    Sprintf(buf, "gem called %s", un);
                else
                    Sprintf(buf, "gems called %s", un);
                pl = 0;
            } else
#endif
        ...
        break;
    default:
        Sprintf(buf, "glorkum %c (0%o) %u %d", obj->olet, obj->olet,
                obj->otyp, obj->spe);
    }
    if (pl) {
        register char *p;
...
register struct obj *obj;
{
    ...
    case TOOL_SYM:                     /* temp. hack by GAN 11/18/86 */
        ...
        if (!(obj->otyp == MAGIC_MARKER || obj->otyp == LAMP))
            break;
