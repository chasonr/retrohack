...
dogushforth() /* Gushing forth in this room */
{
    ...
    if (croom->hx < 0 || has_upstairs(croom) || has_dnstairs(croom)) {
...
drinkfountain()
{
    ...
    if (Levitation)
        pline("You are floating high above the fountain.");
    else if (fate < 10) {
...
register struct obj *obj;
{
    ...
    if (Levitation)
        ...
        if (!obj->rustfree &&
            ...
            (obj->otyp == LONG_SWORD || obj->otyp == KATANA
             || obj->otyp == BROAD_SWORD || obj->otyp == SHORT_SWORD
             || obj->otyp == TWO_HANDED_SWORD)) {
            if (obj->spe > -6) {
                pline("Your weapon rusts somewhat.");
                obj->spe--;
            } else
                pline("Your weapon looks quite rusted.");
        } else
        ...
        if (obj->otyp == LONG_SWORD
            ...
            /* Be *REAL* nice to him */
            pline("A murky hand from the depths reaches up to bless the "
                  "sword.");
