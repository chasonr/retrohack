...
void name_file(file, level) char *file;
int level;
{
    char *tf;
...
long freediskspace(path) char *path;
{
    union REGS regs;
...
copybones(mode)
{
    ...
    if (last[0]) {
        ...
        comspec = getcomspec();
        status =
            spawnl(P_WAIT, comspec, comspec, copy, from, to, "> nul", NULL);
    } else
        return;
 ...
 */
#define SETPCHAR(f, n) \
    showsyms.f = (strlen(translate) > n) ? translate[n] : defsyms.f
            SETPCHAR(stone, 0);
            SETPCHAR(vwall, 1);
            SETPCHAR(hwall, 2);
            SETPCHAR(tlcorn, 3);
