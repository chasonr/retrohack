...
register int x, y;
{
    ...
    if (ep && ep->engr_txt[0]) {
        ...
        if (canfeel || !Blind)
            pline("You %s: \"%s\".", (Blind) ? "feel the words" : "read",
                  ep->engr_txt);
    }
}
...
register char *s;
{
    ...
    if (ep = engr_at(x, y))
        del_engr(ep);
    ep = (struct engr *) alloc(
        (unsigned) (sizeof(struct engr) + strlen(s) + 1));
...
doengrave()
{
    ...
    if (!otmp)
        return (0);
                 ...
                 * while both your hands are tied up.  Also, it's necessary  \
                 * to prevent engraving with "worn" objects other than                            \
                 * weapons.                                                  \
                 */
    ...
    if (otmp == &zeroobj) {
        ...
        if ((objects[otmp->otyp].bits & NODIR)) {
            ...
            case WAN_LIGHTNING:
                if (!objects[otmp->otyp].oc_name_known) {
                    pline("The %s is a wand of lightning!", xname(otmp));
            ...
            case WAN_FIRE:
                if (!objects[otmp->otyp].oc_name_known) {
                    pline("The %s is a wand of fire!", xname(otmp));
                    objects[otmp->otyp].oc_name_known = 1;
                    more_experienced(0, 10);
                }
                type = BURN;
            ...
            case WAN_DIGGING:
                if (!objects[otmp->otyp].oc_name_known) {
                    pline("The %s is a wand of digging!", xname(otmp));
                    objects[otmp->otyp].oc_name_known = 1;
                    more_experienced(0, 10);
                }
                type = ENGRAVE;
        ...
        if (type == DUST)
            pline("You write in the dust with %s.", doname(otmp));
        ...
        if (otmp->otyp == DAGGER || otmp->otyp == TWO_HANDED_SWORD
            ...
            || otmp->otyp == SHORT_SWORD || otmp->otyp == LONG_SWORD
            || otmp->otyp == AXE) {
            type = ENGRAVE;
            if ((int) otmp->spe <= -3) {
...
#endif
        default:
            impossible("\"%s\" is now written in a very strange way.", buf);
        }
    } else {
        pline("What do you want to %s on the floor here? ",
              (type == ENGRAVE) ? "engrave"
                                : (type == BURN) ? "burn" : "write");
        getlin(buf);
        clrlin();
    }
    spct = 0;
    ...
    if (otmp->otyp == WAN_FIRE) {
        if (!Blind)
            pline("Flames fly from the wand.");
        else
            pline("You feel the wand heat up.");
    ...
    case WAN_COLD:
        ...
        break;
    case WAN_STRIKING:
        pline("The %s unsuccessfully fights your attempt to write!",
              xname(otmp));
    }
