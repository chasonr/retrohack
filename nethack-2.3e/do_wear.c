...
doremarm()
{
...
#ifdef SHIRT
    otmp = (!uarmh && !uarms && !uarmg)
               ? (uarm ? uarm : uarmu)
               : (!uarm && !uarms && !uarmg && !uarmu)
                     ? uarmh
                     : (!uarm && !uarmh && !uarmg && !uarmu)
                           ? uarms
                           : (!uarm && !uarmh && !uarms && !uarmu)
                                 ? uarmg
                                 :
#else
    otmp =
    ...
    if (otmp == uarmg && uwep && uwep->cursed) { /* myers@uwmacc */
        pline("You seem not able to take off the gloves while holding your "
              "weapon.");
        return (0);
    }
    (void) armoroff(otmp);
...
corrode_armor()
{
    ...
    if (otmph) {
        if (otmph->rustfree || otmph->otyp == CRYSTAL_PLATE_MAIL
