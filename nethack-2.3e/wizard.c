            ...
            for (otmp = invent; otmp; otmp = otmp->nobj)
                if (otmp->olet == AMULET_SYM && !otmp->spe) {
                    ...
                    if (dist(mtmp->mx, mtmp->my) > 2)
                        pline("You get the creepy feeling that somebody "
...
/* For giants throwing rocks, the rock which hits you shouldn't disappear. */
            ...
            if (!thitu(8, rnd(oclass->wldam), oclass->oc_name)) {
                ...
                if (hit && obj->otyp == CREAM_PIE) {
                    if (!Blind)
                        pline("Yeech! You've been creamed.");
                    else
                        pline("There's something sticky all over your face.");
...
register struct monst *mtmp;
{
    ...
    if ((!tx || !ty || abs(tx) == abs(ty)) /* straight line or diagonal */
        ...
        case 'C':
            ...
            y = mtmp->my;
#ifdef KOPS
            otmp = (mtmp->data->mlet == 'O')
                       ? m_carrying(mtmp, DART)
#ifdef KAA
                       : (mtmp->data->mlet == 'K')
                             ? m_carrying(mtmp, CREAM_PIE)
#endif
#else
            otmp = (mtmp->data->mlet == 'K')
                       ? m_carrying(mtmp, DART)
#endif
#ifdef KAA
                             : (mtmp->data->mlet == '9')
                                   ? m_carrying(mtmp, ENORMOUS_ROCK)
#endif
                                   : m_carrying(mtmp, CROSSBOW_BOLT);
            if (otmp
                && (!URETREATING(x, y)
                    || !rn2(BOLT_LIM - movedist(x, u.ux, y, u.uy)))) {
                m_throw(mtmp->mx, mtmp->my, sgn(tx), sgn(ty), BOLT_LIM, otmp);
                if (!--otmp->quan)
                    m_useup(mtmp, otmp);
                return (0);
            }
    ...
    if ((!tx && abs(ty) < BOLT_LIM) || (!ty && abs(tx) < BOLT_LIM)
        ...
        case 'D':
            ...
            buzz((int) -10 - (mtmp->dragon), mtmp->mx, mtmp->my, sgn(tx),
                 sgn(ty));
            break;
#ifdef HARD
        case '&':
...
intervene()
{
    ...
    case 1:
        ...
        break;
    case 2:
        pline("You notice a black glow surrounding you.");
        rndcurse();
        break;
    case 3:
        aggravate();
