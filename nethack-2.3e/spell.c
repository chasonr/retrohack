...
getspell()
{
    ...
    if (spl_book[0].sp_id == NO_SPELL) {
        ...
        for (max = 1; (max < MAXSPELL) && (spl_book[max].sp_id != NO_SPELL);
             max++)
            ;
        if (max >= MAXSPELL) {
        ...
        for (i = 0; (i < max) && (i < 26); buf[++i] = 0)
            buf[i] = 'a' + i;
        for (i = 26; (i < max) && (i < 52); buf[++i] = 0)
            buf[i] = 'A' + i - 26;

        if (max == 1)
            strcpy(lets, "a");
        else if (max < 27)
            sprintf(lets, "a-%c", 'a' + max - 1);
        else if (max == 27)
            sprintf(lets, "a-z A");
        else
            sprintf(lets, "a-z A-%c", 'A' + max - 27);
        for (;;) {
...
dovspell()
{
    ...
    for (i = 1; i <= max; i++) {
        (void) sprintf(buf, "%c - %s (%d)", spellet(i), spellname(i),
                       spellev(i));
        cornline(1, buf);
        any[i - 1] = spellet(i);
    }
