...
#define FOOD(name, prob, delay, weight, nutrition)                   \
    {                                                                \
        name, NULL, NULL, 1, 1, FOOD_SYM, prob, delay, weight, 0, 0, \
            nutrition                                                \
    }
...
#define WEAPON(name, prob, wt, sdam, ldam)                                  \
    {                                                                       \
        name, NULL, NULL, 1, 0 /*%%*/, WEAPON_SYM, prob, 0, wt, sdam, ldam, \
            0                                                               \
    }

    /* Note: for weapons that don't do an even die of damage (i.e. 2-7 or
     * 3-18) the extra damage is added on in fight.c, not here! */
...
/* Because giants can throw rocks */
    ...
    { "enormous rock", NULL, NULL, 1, 0, ROCK_SYM, 100, 0,
      200 /* > MAX_CARR_CAP */, 0, 20, 20 },
#else
    { "enormous rock", NULL, NULL, 1, 0, ROCK_SYM, 100, 0,
      250 /* > MAX_CARR_CAP */, 0, 0, 0 },
#endif
#else
#ifdef KAA
    { "enormous rock", NULL, NULL, 1, 0, ROCK_SYM, 100, 0,
      400 /* > MAX_CARR_CAP */, 0, 20, 20 },
#else
    { "enormous rock", NULL, NULL, 1, 0, ROCK_SYM, 100, 0,
      550 /* > MAX_CARR_CAP */, 0, 0, 0 },
#endif
#endif

#define ARMOR(name, prob, delay, weight, ac, can)                          \
    {                                                                      \
        name, NULL, NULL, 1, 0, ARMOR_SYM, prob, delay, weight, ac, can, 0 \
    }
    /* Originally, weight was always 8, which is ridiculous.  (Plate mail
       weighs the same as a pair of gloves?) */
...
#ifdef SPELLS
/* books */
#define SPELL(name, desc, prob, delay, flags, level)                        \
    {                                                                       \
        name, desc, NULL, 0, 0, SPBOOK_SYM, prob, delay, 5, flags, 0, level \
    }
...
char obj_symbols[] = { ILLOBJ_SYM, AMULET_SYM, FOOD_SYM,   WEAPON_SYM,
                       TOOL_SYM,   BALL_SYM,   CHAIN_SYM,  ROCK_SYM,
                       ARMOR_SYM,  POTION_SYM, SCROLL_SYM, WAND_SYM,
#ifdef SPELLS
                       SPBOOK_SYM,
#endif
                       RING_SYM,   GEM_SYM,    0 };
