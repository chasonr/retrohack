...
register struct trap *trap;
{
    ...
    if (trap->tseen && !rn2(5)
        ...
        case TRAPDOOR:
            if (!xdnstair) {
                pline("A trap door in the ceiling opens and a rock falls on "
                      "your head!");
                if (uarmh)
        ...
        case RUST_TRAP:
            ...
            glovecheck:
                if (uarmg)
                    pline("Your gloves are not affected!");
                break;
            case 2:
            ...
            default:
                ...
                if (uarm) {
                    if (uarm->rustfree || uarm->otyp >= STUDDED_LEATHER_ARMOR)
                        pline("Your %s not affected!", aobjnam(uarm, "are"));
                    else if (uarm->spe > -6) {
...
register struct monst *mtmp;
{
    ...
    if (!trap) {
         ...
         */
        int in_sight =
            cansee(mtmp->mx, mtmp->my) && (!mtmp->minvis || See_invisible);
#else
        ...
        case TRAPDOOR:
            if (!xdnstair) {
                mtmp->mhp -= 10;
                if (in_sight)
                    pline("A trap door in the ceiling opens and a rock hits "
                          "%s!",
                          monnam(mtmp));
                break;
            }
        ...
        case SQBRD: {
            ...
            if (index("EyBIfk", mtmp->data->mlet))
                break;
            /* Stepped on a squeaky board. */
            if (in_sight)
        ...
        case LANDMINE: {
            ...
            if (in_sight)
                pline("KAABLAMM!!! %s steps on a land mine!", Monnam(mtmp));
            else
...
#ifdef KAA
        if (multi < 0
            && (!nomovemsg || !strncmp(nomovemsg, "You awake", 9)
...
domagictrap()
{
    ...
    if (fate < 10) {
        ...
        /* odd feelings */
        case 13:
            pline("A shiver runs up and down your spine!");
            break;
        case 14:
            pline("You hear distant howling.");
            break;
        case 15:
            pline("You suddenly yearn for your distant homeland.");
            break;
        case 16:
