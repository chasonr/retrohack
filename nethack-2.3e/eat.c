    ...
    if (!rn2(7) && otmp->otyp != FORTUNE_COOKIE) {
        ...
        case FOOD_RATION:
            ...
            lesshungry(ftmp->nutrition);
                ...
                pline(
                    "You're having a hard time getting all that food down.");
                multi -= 2;
            }
            lesshungry(ftmp->nutrition);
            if (multi < 0)
        ...
        case WEAK:
            if (Hallucination)
                pline((!incr) ? "You still have the munchies."
                              : "The munchies are starting to interfere with "
                                "your motor capabilities.");
            else
...
register struct obj *otmp;
{
    ...
    case 'M':
        if (u.usym == '@') {
            pline("You cannot resist the temptation to mimic a treasure "
                  "chest.");
            tp++;
            nomul(-30);
            afternmv = Meatdone;
