...
seemons()
{
    ...
    for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
        if (mtmp->data->mlet == ';')
            mtmp->minvis =
                (u.ustuck != mtmp && levl[mtmp->mx][mtmp->my].typ == POOL);
        pmon(mtmp);
#ifndef NOWORM
        if (mtmp->wormno)
            wormsee(mtmp->wormno);
...
bot()
{
    ...
    (void) sprintf(newbot,
                   ...
                   u.mtimedone ? u.mhmax : u.uhpmax, u.uen, u.uenmax, u.uac);
#else
                   u.uhp, u.uhpmax, u.uen, u.uenmax, u.uac);
#endif
#else
                   "Level %-2d  Gold %-5lu  Hp %3d(%d)  Ac %-2d  ", dlevel,
                   u.ugold,
#ifdef KAA
                   u.mtimedone ? u.mh : u.uhp,
    ...
    (void) sprintf(
        eos(newbot), " S:%lu ",
        (u.ugold - u.ugold0 > 0 ? u.ugold - u.ugold0 : 0) + u.urexp
            + (50 * maxdlevel)
            + (maxdlevel > 20
                   ? 1000 * ((maxdlevel > 30) ? 10 : maxdlevel - 20)
                   : 0));
#endif
...
register struct monst *mtmp;
{
    ...
    pline("Level %-2d  Gold %-5lu  Hp %3d(%d)", mtmp->data->mlevel,
          mtmp->mgold, mtmp->mhp, mtmp->mhpmax);
    pline("Ac %-2d  Dam %d %s %s", mtmp->data->ac,
...
rndmonsym()
{
    ...
    if ((x = rn2(58)) < 26)
        ...
        case 57:
            return ('9');
        default:
            impossible("Bad random monster %d", x);
            return ('{');
        }
}
