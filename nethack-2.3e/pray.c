...
#define STARVED 6
    ...
    if ((uarmh && uarmh->cursed)    /* helmet */
        ...
        || (uleft && uleft->cursed) || (uright && uright->cursed))
    ...
    if (Blinded > 1)
        return (TROUBLE_BLIND); /* don't loop when fixing sets to 1 */
    if (u.ustr < u.ustrmax)
...
fix_worst_trouble()
{
    ...
    case TROUBLE_STARVING:
        pline("Your stomach feels content.");
        init_uhunger();
        break;
    case TROUBLE_SICK:
        ...
        break;
    case TROUBLE_HIT:
        pline("A %s glow surrounds you", Hallucination ? hcolor() : "golden");
        u.uhp = u.uhpmax += 5;
        break;
...
dosacrifice()
{
    ...
    if (value > 0) {
        ...
        if (Hallucination)
            ...
            pline("Your sacrifice is consumed in a burst of flame!");
        if (u.ugangr) {
            ...
            if (u.ugangr != saved_anger)
                if (u.ugangr) {
                    if (Hallucination)
                        ...
                        pline("The gods seem slightly molified.");
                } else {
                    if (Hallucination)
            ...
            if (u.ublesscnt != saved_cnt) {
                ...
                if (u.ublesscnt) {
                    if (Hallucination)
                        pline("You realize that the gods are not like you "
                              "and I.");
                    else
        ...
        } else /* you were already in pretty good standing */
        {
            ...
            if (u.uluck != saved_luck) {
                if (Hallucination)
                    pline("You see crabgrass at your feet. A funny thing in "
                          "a dungeon.");
                else
                    pline("You see a four-leaf clover at your feet.");
            }
        }
    }
    useup(otmp);
...
#endif
    } else if ((int) u.uluck < 0)
        angrygods(); /* a bad player */
    else
        pleased(); /* or a good player */
#endif
...
angrygods()
{
    ...
    case 0:
    case 1:
        pline("but nothing appears to happen.");
        break;
    case 2:
    case 3:
        pline("A voice booms out: You are arrogant, mortal.");
    ...
    case 7:
    case 8:
        pline("A voice booms out: You dare to call upon us?");
...
pleased()
{
    ...
    if (!trouble)
        ...
        if (action < 4 && action >= 2)
            while (in_trouble() > 0)
                fix_worst_trouble();
        if (action == 1 && trouble > 0)
            fix_worst_trouble();
        if (action == 0)
            pline("but nothing seems to happen.");
    };
    if (pat_on_head)
        pline("The gods seem especially pleased ...");
    ...
    if (pat_on_head)
        ...
        case 4:
        case 5:
            pline("A voice booms out: We are pleased with your progress,");
        ...
        case 6:
            pline("An object appears at your feet!");
#ifdef SPELLS
            mkobj_at('+', u.ux, u.uy);
#else
        mkobj_at('?', u.ux, u.uy);
        ...
        case 7:
            pline("A voice booms out:  We crown thee...");
