...
register struct monst *mtmp;
{
    ...
    if (u.uswallow) {
        ...
        case ',':
            youswld(mtmp, (u.uac > 0) ? u.uac + 4 : 4, 5, Monnam(mtmp));
            break;
        case '\'':
    ...
    ctmp =
        tmp && !mtmp->mcan && (!uarm || objects[uarm->otyp].a_can < rnz(3));
    switch (mdat->mlet) {
    case '1':
        if (wiz_hit(mtmp))
    ...
    case 'D':
        ...
        buzz((int) -10 - (mtmp->dragon), mtmp->mx, mtmp->my, u.ux - mtmp->mx,
             u.uy - mtmp->my);
        break;
    case 'd':
    ...
    case 'N':
        ...
        if (u.usym == 'N') {
            if (mtmp->minvent)
                pline("%s brags about the goods some dungeon explorer "
                      "provided.",
                      Monnam(mtmp));
            else
                pline("%s makes some remarks about how difficult theft is "
                      "lately.",
                      Monnam(mtmp));
            rloc(mtmp);
        } else
    ...
    case 'Q':
        ...
        break;
    case 'R':
        if (ctmp && uarmh && !uarmh->rustfree && (int) uarmh->spe >= -1) {
...
#ifdef KAA
        if (mtmp->mcan)
            pline("%s nuzzles against your %s leg!", Monnam(mtmp),
                  (side == RIGHT_SIDE) ? "right" : "left");
        else {
#endif
            pline("%s pricks your %s leg!", Monnam(mtmp),
                  (side == RIGHT_SIDE) ? "right" : "left");
...
register struct monst *mtmp;
{
    ...
    if (!strcmp(mtmp->data->mname, "demon prince") && mtmp->minvis) {
        if (!Blind)
            pline("%s appears before you.", Xmonnam(mtmp));
        mtmp->minvis = 0;
        pmon(mtmp);
    }
    if (u.usym == '&') { /* Won't blackmail their own. */

        pline("%s says, 'Good hunting %s.' and vanishes", Xmonnam(mtmp),
    ...
    if (!demand) { /* you have no gold */
        ...
        pline("%s demands %d Zorkmids for safe passage.", Xmonnam(mtmp),
              demand);
        ...
        if (offer >= u.ugold) {
            ...
            pline("You give %s %d Zorkmids.", xmonnam(mtmp, 0), offer);
        u.ugold -= offer;
