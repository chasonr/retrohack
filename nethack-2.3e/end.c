...
register struct monst *mtmp;
{
    ...
    if (mtmp->data->mlet == ' ') {
        ...
        Sprintf(eos(buf), "%s %s, the shopkeeper)", (rn2(2) ? "Mr." : "Ms."),
                shkname(mtmp));
    } else
        Sprintf(eos(buf), "%s", mtmp->data->mname);
    if (mtmp->mnamelth)
...
die:
...
#ifdef DGKMOD
                i = (long) otmp->quan * objects[otmp->otyp].g_val;
#else
    ...
    if (!done_stopprint)
        printf("You were level %u with a maximum of %d hit points when you "
               "%s.\n",
