...
register struct monst *mtmp;
{
    ...
    if (!nearby || mtmp->mflee || scared || mtmp->mconf
        || (mtmp->minvis && !rn2(3)) ||
#ifndef KOPS
        ...
        if (Hallucination && tmp == 0)
            pmon(mtmp);
            /* If 0, this means the monster didn't move.  During
               hallucination, its appearance should still change. */
...
#define SRCHRADIUS 25
    {
        ...
        if (likegems || likeobjs
            ...
            for (otmp = fobj; otmp; otmp = otmp->nobj)
                if (likeobjs || (likegems && otmp->olet == GEM_SYM)
#ifdef KAA
                    || (likerock && otmp->olet == ROCK_SYM)
#endif
                ) {
                    if (msym != 'u' || objects[otmp->otyp].g_val != 0)
                        if ((dd = DIST(omx, omy, otmp->ox, otmp->oy))
                            < mind) {
                            mind = dd;
                            gx = otmp->ox;
                            gy = otmp->oy;
...
#endif
                                  (msym == '@' || msym == '1')
                                      ? (ALLOW_SSM | ALLOW_TRAPS)
                                      : index(UNDEAD, msym)
                                            ? NOGARLIC
                                            :
#ifdef KAA
                                            (msym == '9')
                                                ? (ALLOW_ROCK | ALLOW_TRAPS)
                                                : ALLOW_TRAPS);
#else
                                        ALLOW_TRAPS);
#endif
